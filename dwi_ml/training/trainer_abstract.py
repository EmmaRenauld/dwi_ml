...
class DWIMLTrainer:
    ...
    This Trainer class's train() method:
        ...
        sampler.source_data.
        - Trains each epoch by using compte_batch_loss, which should be
    ...
    def train(self, *args):
        ...
        for epoch in iter_timer(range(self.current_epoch, self.max_epochs)):
            ...
            # Training
            self._train_one_epoch(train_dataloader, train_context, epoch,
                                  *args)

            # Validation
            self._validate_one_epoch(valid_dataloader, valid_context, epoch,
                                     *args)
    ...
    def _train_one_epoch(self, train_dataloader, train_context, epoch, *args):
        """
        Train one epoch of the model: loop on all batches.

        All *args will be passed all to train_one_batch, in case you want to
        overwrite it and have this method call it correctly.
        """
    ...
    def _validate_one_epoch(self, valid_dataloader, valid_context, epoch,
        ...
        with tqdm(valid_dataloader, ncols=100, disable=self.taskman_managed,
            ...
            for step, data in valid_iterator:
                # Break if maximum number of epochs has been reached
                if step == self.nb_valid_batches_per_epoch:
    ...
    def init_from_checkpoint(cls, batch_sampler_training: BatchSamplerAbstract,
                             batch_sampler_validation: BatchSamplerAbstract,
                             model: ModelAbstract, checkpoint_state: dict):
        """
