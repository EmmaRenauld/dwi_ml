...
from dwi_ml.general.unit_tests.utils.expected_values import (
    TEST_EXPECTED_VOLUME_GROUPS, TEST_EXPECTED_STREAMLINE_GROUPS,
    TEST_EXPECTED_SUBJ_NAMES)
from dwi_ml.general.unit_tests.utils.data_and_models_for_tests import fetch_testing_data
...
def test_help_option(script_runner):
    ret = script_runner.run('tt_train_model', '--help')
    assert ret.success

    ret = script_runner.run(
        'tt_resume_training_from_checkpoint', '--help')
    assert ret.success

    ret = script_runner.run('tt_track_from_model', '--help')
    assert ret.success

    ret = script_runner.run('tt_visualize_loss', '--help')
    assert ret.success
